...
class LaserScanVis:
  ...
  def __init__(self, scan, scan_names, label_names, offset=0,
    ...
    self.reset() 
    # self.update_scan() //modified by dhz, 2022.7.4, commented for remote display

    rospy.init_node("create_cloud_xyzrgb")
    self.pub = rospy.Publisher("point_cloud2", PointCloud2, queue_size=2)

  def append_scan_names(self,new_scan_names):
    self.scan_names = new_scan_names

  def append_label_names(self,new_label_names):
    self.label_names = new_label_names
  
  def update_scan_and_broadcast(self):
    self.offset = len(self.label_names) - 1
    ...
    print(self.scan.sem_label_color.shape)
    points = []
    for i in range(len(self.scan.points)):
      x = self.scan.points[i][0]
      y = self.scan.points[i][1]
      z = self.scan.points[i][2]
      b = int(self.scan.sem_label_color[i][0] * 255.0)
      g = int(self.scan.sem_label_color[i][1] * 255.0)
      r = int(self.scan.sem_label_color[i][2] * 255.0)
      a = 255
  ...
  def update_scan_and_display(self):
    self.offset = len(self.label_names) - 1
    self.scan.open_scan(self.scan_names[self.offset])
    if self.semantics:
      self.scan.open_label(self.label_names[self.offset])
